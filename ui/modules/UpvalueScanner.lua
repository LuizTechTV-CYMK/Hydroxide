...
local function addElement(upvalueLog, upvalue, index, value, temporary)
    ...
    elementLog.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and not ctrlHeld then
            touchHeld = true
        end
    end)
    
    elementLog.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and touchHeld then
...
local function addUpvalue(upvalue, temporary)
    ...
    upvalueLog.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and not ctrlHeld then
            touchHeld = true
        end
    end)
    
    upvalueLog.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and touchHeld then
            RunService.Heartbeat:Wait()
            selectedUpvalue = upvalue
            selectedUpvalueLog = upvalueLog
